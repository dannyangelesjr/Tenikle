...
(function () {
    ...
    for (var checkoutId, key, i = 0; i < localStorage.length; ++i) {
      key = localStorage.key(i); if (key.match(shopifyDomain + '.checkoutId')) {
        ...
        if (checkoutId == null) {
          ...
          alert('cart found. checking variant if in cart');
          // check if product variant already exists in cart
          client.checkout.fetch(checkoutId).then((checkout) => {
            if (checkout.lineItems.length > 0) {
              alert('variant ' + variantId + ' found in cart');
              ...
              checkout.lineItems.forEach((lineItem) => {
                if (variantId == lineItem.variant.id) {
                  ...
                  isVariantFound = true;
                  alert('updated variant ' + variantId + ' in cart');
                }
              });
              if (!isVariantFound) {
                alert('variant ' + variantId + ' not found in cart');
                addLineItem(client, checkoutId, productId, variantId, quantity);
                alert('added variant  ' + variantId + ' to cart');
              }
            }
            else {
              ...
              addLineItem(client, checkoutId, productId, variantId, quantity);
              alert('discount applied ' + variantId);
            }
          })
        };
      }
  ...
  function addLineItem(client, checkoutId, productId, variantId, quantity) {
    alert('adding cart lineitem' + checkoutId + ',' + productId + ',' + variantId);
    let lineItem = [
      {
    ...
    alert('lineitem added ' + checkoutId + ',' + productId + ',' + variantId);
  }

  function updateLineItem(client, checkoutId, lineItemId, productId, variantId, quantity) {
    alert('updating cart lineitem' + checkoutId + ',' + productId + ',' + variantId);
    let lineItem = [
      {
        id: btoa(lineItemId),
        quantity: quantity
    ...
    alert('lineitem updated' + checkoutId + ',' + productId + ',' + variantId);
  }

  function updateView(client, checkoutId, productId) {

    alert('updating view ' + checkoutId + ',' + productId);
    ...
    try {
      ...
      alert('view updated ' + checkoutId + ',' + productId + ',');
    }
    catch (ex) { 
      alert(ex);
    }
